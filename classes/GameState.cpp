...
void GameState::init(const char* newState, char player) {
    ...
    // White king and rooks in starting positions
    if (state[4] == 'K') {
        if (state[0] == 'R') flags |= WhiteQueenSide;
        if (state[7] == 'R') flags |= WhiteKingSide;
    }

    // Black king and rooks in starting positions
    if (state[60] == 'k') {
        if (state[56] == 'r') flags |= BlackQueenSide;
        if (state[63] == 'r') flags |= BlackKingSide;
    }
...
void GameState::generateKingMoves(std::vector<BitMove>& moves, BitBoard piecesBoard, uint64_t occupancy) {
    piecesBoard.forEachBit([&](int fromSquare) {
        ...
        uint64_t friendlyPieces = (color == WHITE)
            ? _bitboards[WHITE_ALL_PIECES].getData()
            : _bitboards[BLACK_ALL_PIECES].getData();
        ...
        if ((flags & QueenSideCastle) && fromSquare == (color == WHITE ? 4 : 60)) {
            legalMoves &= ~(1ULL << (color == WHITE ? 2 : 58));
        }

        BitBoard moveBits(legalMoves);
        moveBits.forEachBit([&](int toSquare) {
            ...
            (!white && fromSquare == 60))
        {
            ...
            if (flags & KingSideCastle)
            {
                if (white)
                {
                    ...
                    if (state[5] == '0' && state[6] == '0' &&
                        !isSquareAttacked(4, enemyColor, _bitboards) &&
                        !isSquareAttacked(5, enemyColor, _bitboards) &&
                        !isSquareAttacked(6, enemyColor, _bitboards))
                    {
                        moves.emplace_back(4, 6, King, MoveFlags::KingSideCastle);
                    }
                }
                else
                {
                    ...
                    if (state[61] == '0' && state[62] == '0' &&
                        !isSquareAttacked(60, enemyColor, _bitboards) &&
                        !isSquareAttacked(61, enemyColor, _bitboards) &&
                        !isSquareAttacked(62, enemyColor, _bitboards))
                    {
                        moves.emplace_back(60, 62, King, MoveFlags::KingSideCastle);
                    }
                }
            }
            ...
            if (flags & QueenSideCastle)
            {
                if (white)
                {
                    // d1=3, c1=2, b1=1 must be empty
                    if (state[3] == '0' && state[2] == '0' && state[1] == '0' &&
                        !isSquareAttacked(4, enemyColor, _bitboards) &&
                        !isSquareAttacked(3, enemyColor, _bitboards) &&
                        !isSquareAttacked(2, enemyColor, _bitboards))
                    {
                        moves.emplace_back(4, 2, King, MoveFlags::QueenSideCastle);
                    }
                }
                else
                {
                    // d8=59, c8=58, b8=57 must be empty
                    if (state[59] == '0' && state[58] == '0' && state[57] == '0' &&
                        !isSquareAttacked(60, enemyColor, _bitboards) &&
                        !isSquareAttacked(59, enemyColor, _bitboards) &&
                        !isSquareAttacked(58, enemyColor, _bitboards))
                    {
                        moves.emplace_back(60, 58, King, MoveFlags::QueenSideCastle);
                    }
                }
            }
        }
...
std::vector<BitMove> GameState::generateAllMoves()
{
    ...
    int pawnIdx   = (color == WHITE) ? WHITE_PAWNS   : BLACK_PAWNS;
    int knightIdx = (color == WHITE) ? WHITE_KNIGHTS : BLACK_KNIGHTS;
    int bishopIdx = (color == WHITE) ? WHITE_BISHOPS : BLACK_BISHOPS;
    int rookIdx   = (color == WHITE) ? WHITE_ROOKS   : BLACK_ROOKS;
    int queenIdx  = (color == WHITE) ? WHITE_QUEENS  : BLACK_QUEENS;
    ...
    generatePawnMoveList(moves, _bitboards[pawnIdx], BitBoard(~_bitboards[OCCUPANCY].getData()), _bitboards[oppAllIdx], color);
    generateKingMoves(moves, _bitboards[kingIdx], ~_bitboards[myAllIdx].getData());
    generateBishopMoves(moves, _bitboards[bishopIdx], _bitboards[OCCUPANCY].getData(), _bitboards[myAllIdx].getData());
    generateRooksMoves(moves, _bitboards[rookIdx], _bitboards[OCCUPANCY].getData(), _bitboards[myAllIdx].getData());
    generateQueensMoves(moves, _bitboards[queenIdx], _bitboards[OCCUPANCY].getData(), _bitboards[myAllIdx].getData());
